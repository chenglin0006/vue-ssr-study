<template>
  <div class="dropdown v-select" :class="dropdownClasses">
    <div ref="toggle" class="dropdown-toggle clearfix" type="button">
      ...
      <input
              ...
              @focus="setAppHeight"
      >
      <img class="clear-all-input" @click="clearAllInputFun" src="../img/clear_input.png" v-show="this.search">
    ...
    <ul ref="dropdownMenu" :transition="transition" class="dropdown-menu" :style="{ 'max-height': maxHeight }">
      <li v-for="(option, index) in filteredOptions" v-bind:key="index" :class="{ active: isOptionSelected(option)}" @click="select(option)">
        <div v-html="getOptionLabel(option)">
          <!-- {{ getOptionLabel(option) }} -->
        </div>
      </li>
      ...
      <transition name="fade">
        <li v-if="!filteredOptions.length" class="text-center">
          <slot v-if="nullText" name="no-options">{{nullText}}</slot>
          <slot v-else name="no-options">没有找到匹配项</slot>
        </li>
      </transition>
    </ul>
  </div>
...
import $ from 'zepto';
  ...
  export default {
    ...
    computed: {
       ...
       */
      searchPlaceholder() {
        if (this.isValueEmpty && this.placeholder || this.search==''&& this.placeholder) {
...
<style>
  ...
  .v-select .dropdown-toggle {
    ...
    padding: 0;
    background: none;
    border: .1rem solid rgba(60, 60, 60, .26);
    border-radius: .4rem;
    white-space: normal;
    position: relative;
  }
